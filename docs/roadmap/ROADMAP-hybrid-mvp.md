...
status: "draft"
version: "0.8.0+draft"
content_hash: "atom:bfa735ce6c7dae74"
updated: "2026-06-25"
owner: "LYRA"
auditor: "ATHER"
...
| PHASE-HYB-05 | LATER: distribution + GTM (npm, web meter, Thai/SEA) | SYSTEM-05 | landing, CR | published; web cost-meter view; first Thai/SEA motion | in_progress | 80 |
...
| SPR-HYB-03 | PHASE-HYB-02 | V1 hardening: onboarding + cross-platform + per-language routing | 2 | a stranger can run it on their own repo | done | 100 |
...
| TASK-HYB-RM-006 | SPR-HYB-03 | feature | Onboarding: detect/install Ollama, pull a default general coding model, accept a frontier key/subscription | SYSTEM-05 | P0 | KIN | V1 onboarding | TASK-HYB-RM-005 | first run sets up deps or degrades gracefully with a clear message | done | 100 |
...
| TASK-HYB-RM-010 | PHASE-HYB-05 | epic | Distribution: npm publish + web cost-meter view (V0.5) + Thai/SEA beachhead | SYSTEM-05 | P2 | LYRA | GTM | TASK-HYB-RM-005 | published; shareable web meter; first Thai/SEA motion | in_progress | 80 |
...
- [x] PHASE-HYB-04 memory moat (FEAT-Per-Agent-Memory-Unit — **all FR contracts implemented + tested; T0/T1 live, T1 distillation invokable**):
  ...
  - Live: `recordOutcome` tags each lesson with tier + role; `distill` is invokable on the real failure-log via `store.distillRole(role)` and the `orchestrator distill [role]` CLI. Tests: `promotion.test.mjs` 5/5 + `memory.test.mjs` 9/9 + `distill-store.test.mjs` 1/1.
...
- [x] **PUBLISHED 2026-06-25**: `hybrid-meter@0.1.0` is live on the npm registry under maintainer `suanranger` — `npm install -g hybrid-meter` / `npx hybrid-meter run "task"` works for anyone. Tarball: https://registry.npmjs.org/hybrid-meter/-/hybrid-meter-0.1.0.tgz (100.4 kB). Remaining for RM-010 = Thai/SEA GTM motion (non-code).

## Changelog

| Version | Date | Owner | Summary |
|---|---|---|---|
| 0.8.0+draft | 2026-06-25 | LYRA | PUBLISHED: hybrid-meter@0.1.0 live on npm registry (maintainer suanranger). RM-010 now blocks only on Thai/SEA GTM motion (non-code). |
