...
|---|---|---|---|
| 0.3.51+draft | 2026-08-14 | ATHER | Merged current main into PR #131 to resolve the stale registry conflict; retained the owner-approved MissionSnapshot registration and the #109/#110/#111/#76 issue-sweep synchronization, with registry entries from both sides preserved. |
| 0.3.50+draft | 2026-08-10 | ATHER | Registered the owner-approved MissionSnapshot orchestration contract and synchronized the production-readiness masterplan to 0.2.5 after TASK-PRD-005 entered review with local QA evidence; ATHER audit remains pending. |
...
| 0.3.35+draft | 2026-08-09 | Claude Fable 5 | Synchronized SPEC-WORKSPACE-SYSTEM to 0.2.1+draft (§3.3 personnel identity model implemented by packages/govibe-core/src/personnel.mjs with the vaults.mjs agent-namespace guard; §6 RBAC still specified-not-implemented) and MASTERPLAN-GOVIBE-PRODUCTION-READINESS to 0.1.10+draft (TASK-PRD-014 executed to review with the 15-test personnel suite). No document status changed to approved/accepted/candidate by this row. |
...
| 0.3.24+draft | 2026-08-06 | Claude (CI-fix session) | Synchronized WP-17-PERSISTENT-MEMORY-MSP-RUNTIME-PHASE-5-STAGE-A to `0.2.4+draft` (post-closure CI fix recorded: a pre-existing two-process concurrent-migration race in `test/shared-scope-fail-closed.security.mjs`, surfaced by PR #121's P0 Security CI `verify` job, fixed by sequencing the test's two spawned processes; `npm run test:security` 65/65 and full gate sweep re-verified). `execution_complete` was already `true` and is unaffected by this row. No document status changed to `approved`/`accepted`/`candidate` by this row. |
| 0.3.23+draft | 2026-08-06 | Boss (CEO) | Synchronized WP-17-PERSISTENT-MEMORY-MSP-RUNTIME-PHASE-5-STAGE-A to `0.2.3+draft` (`execution_complete: true`). Owner reviewed the Execution closure section (194/194 msp-runtime tests, 553+1 root tests, gates green, two documented Deviations) presented in chat and explicitly approved closure ("ปิดงานเลย ตั้ง execution_complete: true"). Recorded honestly as owner review of a single-session self-verified execution, not a separate dispatching/final-gate session's independent reproduction. No document status changed to `approved`/`accepted`/`candidate` by this row. |
...
| 0.3.22+draft (merge: main -> feat/wp-16) | 2026-08-06 | Claude (merge: main -> feat/wp-16) | Merged `main` (PR #118, WP-14 squash-merged, including its post-closure CI fix) into `feat/wp-16-msp-runtime-phase-4` to resolve PR #120's merge conflicts. Registry version renumbered to avoid a `0.3.16+draft` collision between that branch's own WP-15/WP-16/WP-17 history and `main`'s independently-numbered WP-14 CI-fix row. WP-14's registry row taken from `main` (`0.1.3+draft`, includes the CI fix); WP-15/WP-16/WP-17 rows kept at that branch's own newer values. No document content besides version bookkeeping was authored by this row. (Duplicate version label vs. the 0.3.22+draft row above -- `feat/wp-16` and `feat/wp-17` independently reached 0.3.22 before merging into each other; kept distinguishable by owner label rather than silently dropped.) |
...
| 0.3.19+draft (merge: feat/wp-14 -> feat/wp-15) | 2026-08-06 | Claude (merge: feat/wp-14 -> feat/wp-15) | Merged `main` (PR #118, WP-14 squash-merged, including its post-closure CI fix) into `feat/wp-15-msp-runtime-phase-3` to resolve PR #119's merge conflicts. Registry version renumbered to avoid a `0.3.16+draft` collision between that branch's own WP-15/WP-16 history and `main`'s independently-numbered WP-14 CI-fix row. WP-14's registry row taken from `main` (`0.1.3+draft`, includes the CI fix); WP-15/WP-16 rows kept at that branch's own newer values. No document content besides version bookkeeping was authored by this row. (Duplicate version label vs. the 0.3.19+draft row above -- both branches independently reached 0.3.19 before merging into each other; kept distinguishable by owner label rather than silently dropped.) |
...
| 0.3.16+draft (main) | 2026-08-06 | Claude (CI-fix session) | Synchronized WP-14-VAULT-SCOPING-MSP-RUNTIME-ENTITIES to `0.1.3+draft` (post-closure CI fix, owner-directed: `--test-concurrency=1` added to `test:security` scripts; a two-process migration race in this packet's own `test/shared-scope-fail-closed.security.mjs` sequenced). Fixes PR #118's 15-minute `verify` hang. `execution_complete` was already `true` and is unaffected. No document status changed to `approved`/`accepted`/`candidate` by this row. (Merged from `main`; kept at its original `main`-side version label per the merge rows above rather than renumbered, to preserve its literal history.) |
...
| 0.3.12+draft | 2026-08-05 | Claude (final-gate session) | Synchronized WP-14-VAULT-SCOPING-MSP-RUNTIME-ENTITIES to `0.1.2+draft` (`execution_complete: true`) after independent verification of its execution in `packages/msp-runtime`: AC-01 through AC-07 re-run and spot-checked directly by the final-gate session (92/92 tests reproduced, including a direct read of the collision-reproduction test and the vault-scoped migration file), closing the confirmed HIGH-severity cross-agent Global-Private disclosure. Authorization correctly preceded execution for this packet. No document status changed to `approved`/`accepted`/`candidate` by this row. |
| 0.3.11+draft | 2026-08-05 | Boss (CEO) | **ADR-027 accepted.** Synchronized ADR-027-IN-REPO-MSP-RUNTIME-PACKAGE-BOUNDARY to `0.2.0`/`accepted` (owner acceptance, closing ADR-026's Required-follow-up Issue #75). Synchronized ADR-026-MSP-EXTERNAL-RUNTIME-DEPLOYMENT to `0.1.2+draft` (Issue #75 closed in its Required-follow-up list; ADR-026's own `status` remains `proposed`, unaffected by this closure). Synchronized WP-14-VAULT-SCOPING-MSP-RUNTIME-ENTITIES to `0.1.1+draft` (`execution_authorized: true`, owner-authorized before execution, `approval_recorded_at` set — authorization recorded prior to any implementation dispatch, correcting the sequencing gap WP-13 was found to have). No document status changed to `candidate` by this row. |
