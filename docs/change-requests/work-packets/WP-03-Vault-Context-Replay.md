...
The MSP server-side Vault Registry, canonical relation resolver, GKS materialization, and provider-owned KV byte storage are external services. This repository implements and verifies the GoVibe contracts, client boundary, local exact-context retention, observed-link graph, and impact engine. It does not pretend to modify inaccessible parent services.
...
- Unit tests: pending final CI head.
- Docs validation: pending final CI head.
- Typecheck/build: pending final CI head.
- MCP smoke: pending final CI head.
- GitHub Actions run: to be recorded after final successful head.

# Changelog

| Version | Date | Owner | Summary |
|---|---|---|---|
| 1.0.0 | 2026-08-01 | Boss / ATHER | Expanded WP-03 to full vault/context/replay plus Deep Scan links, backlinks, and explainable impact implementation; moved to verification pending. |
