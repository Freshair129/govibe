...
This is a pre-execution, reversible-operation rollback record. No corpus file or metadata has been changed.

| Included disposition | Count | Rollback authority |
|---|---:|---|
| `move_candidate` | 31 | `.file-organizer-map.json` entries (`from` is pre-operation; `to` is proposed target) |
...
The local map follows the Smart File Organizer array contract and intentionally contains only path moves. The eight metadata candidates remain deferred pending an approved exact metadata change and inverse patch; their pre-change snapshots remain in the immutable source result.
