...
| A5 Agent Management | Renders `snapshot.agents` when available; otherwise shows the Agent Select card-deck carousel with active stats, ability tags, EVA media/video switcher from `public/agents/eva`, sequential autoplay 01 -> 02 -> 03 -> 01, mobile single-column layout, and config overlay. | Switch to A5 with no agents; cycle the deck, confirm active stats/media update, open Configure, and verify the EVA video sequence renders. At mobile width, deck cards and media panel must not overflow horizontally. Ingest `agents`; live roster appears and selecting an agent sends `agent.select`. |
