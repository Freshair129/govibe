...
| A5 Agent Management | Renders `snapshot.agents` when available; otherwise shows Agent Select blueprint with roster cards and character console. | Switch to A5 with no agents; select screen and character console appear. Ingest `agents`; live roster appears and selecting an agent sends `agent.select`. |
