...
export class ProviderUnavailableError extends Error {
  constructor(provider) { super(`Executor provider unavailable: ${provider}`); this.name = 'ProviderUnavailableError'; this.code = 'PROVIDER_UNAVAILABLE'; this.provider = provider; }
}

export function createExecutorRegistry(adapters = {}) {
  return {
    inspect() { return PROVIDERS.map((id) => ({ id, available: typeof adapters[id]?.execute === 'function', capabilities: adapters[id]?.capabilities ?? [] })); },
