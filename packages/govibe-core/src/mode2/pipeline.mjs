...
/**
 * Environment-derived fields are excluded from the hashed view of an artifact. The absolute
 * workspace root and filesystem mtimes are recorded in the artifact because they are useful,
 * but they differ between two byte-identical checkouts, so hashing them would make the
 * cross-environment reproducibility claim false while appearing to hold within one machine.
 */
...
/**
 * Content hashes are only recomputed when a file's cheap fingerprint (size + mtime) has
 * changed. Re-hashing the whole tree on every request is exactly the "rescan everything"
 * behaviour the incremental requirement exists to avoid.
 */
async function buildFileIndex({ adapter, store, files, verifyContent }) {
  ...
  for (const file of files) {
    ...
    // size + mtime is a fast-path HINT, not proof of equality: a same-length edit that
    // preserves mtime (coarse-granularity filesystems, `cp -p`, `touch -r`, archive
    // extraction, or two edits inside one mtime tick) is invisible to it. `verifyContent`
    // forces a full re-hash for callers that need the guarantee rather than the fast path.
...
/**
 * Runs the twelve Mode 2 semantic stages against a client-owned workspace.
 *
 * Nothing is written outside `<root>/.govibe/mode2/` — every write goes through the metadata
 * store, which rejects any other target. The scan reads the workspace; it never mutates it.
 */
export async function runMode2Scan({
  ...
  for (const stage of stages) {
    ...
    if (baseline && baseline.inputHash === inputHash && baseline.extractorVersion === stage.extractorVersion) {
      // A record may only be reused when the artifact it points at can actually be loaded.
      // The metadata root is disposable by contract, so a partial cleanup can leave the
      // record behind without its artifact; reusing it would report `complete` with a
      // dangling reference and silently hand later stages a default scope instead of the
      // real inventory. Fall through and re-execute instead.
