...
function assertRelativeMetadataPath(relativePath) {
  ...
  // A colon in any segment is rejected outright. `path.isAbsolute` returns false for the
  // Windows drive-relative form `C:foo`, and on NTFS such a path writes an alternate data
  // stream rather than the file it looks like. Colons have no legitimate use in a metadata
  // path, so denying them keeps this the single enforceable choke point for future callers.
